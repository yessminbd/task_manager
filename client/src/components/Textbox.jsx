import React from 'react';
import clsx from 'clsx';

const Textbox = React.forwardRef(({ type, placeholder, label, className, register, name, error }, ref) => {
    return (
        <div className='w-full flex flex-col gap-1'>
            {label && (
                <label htmlFor={name} className='text-slate-800'>
                    {label} {/* Ajout du texte du label */}
                </label>
            )}
            <input
            
                type={type}
                name={name}
                placeholder={placeholder}
                ref={ref}
                {...register}
                aria-invalid={error ? 'true' : 'false'} // Correction de la valeur aria-invalid
                className={clsx(
                    'bg-transparent px-3 py-2.5 2xl:py-3 border',
                    {
                        'border-gray-100': !error,
                        'border-red-500': error, // Bordure rouge en cas d'erreur
                    },
                    'placeholder-gray-400 text-gray-900 outline-none text-base focus:ring-2 ring-blue-300',
                    className // Ajout de la classe supplémentaire
                )}
            />
            {error && <span className="text-red-500">{error}</span>} {/* Affichage de l'erreur */}
        </div>
    );
});

// Définir le displayName
Textbox.displayName = 'Textbox';

export default Textbox;
